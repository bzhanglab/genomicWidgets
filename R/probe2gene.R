...
probe2gene = function(probeMat, nThread = NULL, use='median', ...){
  ...
  if(use=='median')
    genelevel = myLapply(epicProbe2GeneMappingClean, function(x)apply(probeMat[x,],2,median,na.rm=T))
  else if(use=='mean')
    genelevel = myLapply(epicProbe2GeneMappingClean, function(x)apply(probeMat[x,],2,mean,na.rm=T))
